"""
现在对于 claudecode 的 chaofan 数据，都存放在 source_dir 文件夹中。
每个子文件夹的 /trajectory_api.jsonl 扮演了 trajs_via_logger_file 的作用。
"""
...
DEFAULT_SOURCE_DIR = Path(
    "/home/ywxzml3j/ywxzml3juser23/harbor/jobs-GLM-5-ready/glm5-oraclesolved/jobs-glm5-oraclesolved-cc-1k-success"
)
DEFAULT_IM_OUTPUT = Path(
    "/home/ywxzml3j/ywxzml3juser57/LLaMA-Factory/data/chaofan_glm5_swerebench_oraclesolved_cc_1k.jsonl"
)
DEFAULT_LF_OUTPUT = Path(
    "/home/ywxzml3j/ywxzml3juser57/LLaMA-Factory/data/chaofan_glm5_swerebench_oraclesolved_cc_1k.json"
)

TRAJECTORY_API_REL = Path("trajectory_api.jsonl")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="将 claudecode chaofan 目录下的 trajectory_api.jsonl 转为 IM 数据"
    )
    parser.add_argument(
        "--source-dir",
        type=Path,
        default=DEFAULT_SOURCE_DIR,
        help="含多个实例子目录的根目录（每实例 trajectory_api.jsonl）",
    )
...
def process_one_instance(trajectory_api_path: Path) -> tuple[list[dict], int, int]:
    ...
    tag_instance_records(converted_records, trajectory_api_path.parent.name)
...
def main() -> None:
    ...
    if exclusion_patterns:
        instance_dirs = filter_paths_by_repo(
            ...
            name_func=lambda p: p.name, label="cc-chaofan",
        )
